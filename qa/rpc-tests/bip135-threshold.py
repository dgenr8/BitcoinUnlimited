...
class BIP135ForksTest(ComparisonTestFramework):
    ...
    def __init__(self):
        ...
        self.num_nodes = 1
        self.forks = ["bip135test%d" % i for i in range(0,8) ] # index matches bit number
    ...
    def test_BIP135Thresholds(self):
        ...
        self.tip = int("0x" + node.getbestblockhash(), 0)
        test_blocks = self.generate_blocks(1, 0x20000000)  # do not set bit 1 yet
        ...
        assert_equal(bcinfo['bip135_forks'][self.forks[1]]['status'], 'started')
        assert(self.forks[1] not in tmpl['rules'])
        assert_equal(tmpl['vbavailable'][self.forks[1]], 1)
        assert_equal(tmpl['vbrequired'], 0)
        assert(tmpl['version'] & 0x20000000 + 2**1)
        ...
        assert_equal(self.get_bip135_status(self.forks[1])['status'], 'started')

        # 10 blocks total for bit 3
        test_blocks = self.generate_blocks(9, 0x200000F9)
        yield TestInstance(test_blocks, sync_every_block=False)
        assert_equal(self.get_bip135_status(self.forks[2])['status'], 'started')

        # 75 blocks total for bit 4
        test_blocks = self.generate_blocks(65, 0x200000F1)
        yield TestInstance(test_blocks, sync_every_block=False)
        assert_equal(self.get_bip135_status(self.forks[3])['status'], 'started')

        # 95 blocks total for bit 5
        test_blocks = self.generate_blocks(20, 0x200000E1)
        yield TestInstance(test_blocks, sync_every_block=False)
        assert_equal(self.get_bip135_status(self.forks[4])['status'], 'started')

        # 99 blocks total for bit 6
        test_blocks = self.generate_blocks(4, 0x200000C1)
        yield TestInstance(test_blocks, sync_every_block=False)
        assert_equal(self.get_bip135_status(self.forks[5])['status'], 'started')
        ...
        bcinfo = node.getblockchaininfo()
        assert_equal(bcinfo['bip135_forks'][self.forks[1]]['status'], 'locked_in')
